...
class EvaluationMetrics(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        return f"MRR: {self.mrr:.4f}, Recall@5: {self.recall_at_5:.4f}, NDCG@5: {self.ndcg_at_5:.4f}"
...
class EvaluationReporter:
    ...
    def _print_metrics_table(all_results: list[UseCaseResult]) -> None:
        ...
        # Print table header
        header = f"{'Use Case':<{max_name_width}} | {'MRR':>8} | {'Recall@5':>10} | {'NDCG@5':>8}"
        print(header)
        print("-" * len(header))
        ...
        print(f"{'AVERAGE':<{max_name_width}} | {avg_mrr:>8.4f} | {avg_recall:>10.4f} | {avg_ndcg:>8.4f}")
        print("=" * len(header))
