...
async def search_repository(clean_database):
...
@pytest.mark.asyncio
async def test_search_by_vector_returns_similar_chunks(search_repository, unit_of_work):
    ...
    results = await search_repository.search_by_vector(query_vector, limit=10)
...
@pytest.mark.asyncio
async def test_search_by_vector_respects_limit(search_repository, unit_of_work):
    ...
    query_vector = [1.0] * 384
    results = await search_repository.search_by_vector(query_vector, limit=3)
...
@pytest.mark.asyncio
async def test_search_by_vector_with_similarity_threshold(search_repository, unit_of_work):
    ...
    query_vector = [1.0] * 384
    results = await search_repository.search_by_vector(
        query_vector,
        limit=10,
...
@pytest.mark.asyncio
async def test_search_by_vector_excludes_chunks_without_embeddings(search_repository, unit_of_work):
...
@pytest.mark.asyncio
async def test_search_by_vector_empty_query_raises_error(search_repository):
    ...
    with pytest.raises(ValueError, match="Query vector cannot be empty"):
        await search_repository.search_by_vector([], limit=10)


@pytest.mark.asyncio
async def test_search_by_vector_wrong_dimensions_raises_error(search_repository):
    ...
    with pytest.raises(ValueError, match="Query vector must be 384-dimensional"):
        await search_repository.search_by_vector([1.0] * 128, limit=10)


@pytest.mark.asyncio
async def test_search_by_vector_returns_empty_when_no_chunks(search_repository):
